...
func (ps *PeerStorage) Append(entries []eraftpb.Entry, raftWB *engine_util.WriteBatch) error {
	...
	// Hint1: 遍历entries，用meta.RaftLogKey生成日志key，写入raftWB（meta类型）
	for _, entry := range entries {
		// 生成Raft日志的key：regionId + entry.Index
		logKey := meta.RaftLogKey(ps.region.Id, entry.Index)
		// 调用WriteBatch的SetMeta实例方法
	...
	// Hint2: 删除旧的、索引大于lastIndex的日志（避免冲突）
	for i := lastIndex + 1; i <= prevLastIndex; i++ {
		oldLogKey := meta.RaftLogKey(ps.region.Id, i)
		raftWB.DeleteMeta(oldLogKey) // 删除旧日志的meta key
	}
...
func (ps *PeerStorage) SaveReadyState(ready *raft.Ready) (*ApplySnapResult, error) {
	...
	// Hint2：Handle the hard state if it is NOT empty（且 LastIndex>0 时）
