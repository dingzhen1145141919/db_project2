...
// GroupKeysByRegion 将多个key按照其所属的Region（区域）进行分组。
// 特殊说明：该函数还会返回第一个key对应的区域，这个区域可作为"PrimaryLockKey"（主锁key），
// 需要在其他区域之前提交。
// 参数说明：
//   bo: 用于处理重试的Backoffer对象
//   keys: 需要分组的key数组（字节数组类型）
//   filter: 过滤函数，用于过滤掉不需要的key，入参为当前key和区域起始key，返回bool表示是否过滤
// 返回值说明：
//   map[RegionVerID][][]byte: 以区域版本ID为键，对应区域的key数组为值的分组结果
//   RegionVerID: 第一个key所属的区域版本ID
//   error: 执行过程中出现的错误（若有）
// 辅助函数：内部调用`RegionCache.LocateKey`方法来获取key所属的区域
func (c *RegionCache) GroupKeysByRegion(bo *Backoffer, keys [][]byte, filter func(key, regionStartKey []byte) bool) (map[RegionVerID][][]byte, RegionVerID, error) {
	// 初始化存储分组结果的map，键为区域版本ID，值为该区域下的key数组
